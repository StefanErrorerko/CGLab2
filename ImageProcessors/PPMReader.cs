...
public class PPMReader : IImageReader
{
    ...
    public Image Read()
    {
        ...
        {
            if (reader.ReadLine() != "P3") throw new ImageFormatException("Invalid file format (not PPM)");

            // read image dimensions
            var dimensions = reader.ReadLine()?.Split(' ');
            if (dimensions is null) throw new ImageFormatException("Error occured while reading dimnesions in PPM");
